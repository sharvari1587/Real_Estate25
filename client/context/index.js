...
export const StateContextProvider = ({ children }) => {
  ...
  const buyPropertyFunction = async (buying) => {
    ...
    try {
      ...
      console.info("Contract call success", transaction);
      setLoader(false);
      notifySuccess("Transaction went successfully");
      setCount(count + 1);
      router.push("/author");
      ...
      generateAndDownloadPDF(productID, address, amount, transaction.hash);
...
const generateAndDownloadPDF = (productID, address, amount, transactionHash) => {
  console.log("PDF Function Called:", productID, address, amount, transactionHash); // Debugging
  ...
  doc.text(`Wallet Address: ${address}`, 20, 50);
  doc.text(`Amount Paid: ${amount} ETH`, 20, 60);
  doc.text(`Transaction Hash: ${transactionHash}`, 20, 70);
  doc.text(`Date: ${new Date().toLocaleString()}`, 20, 80);

  // ✅ Ensure PDF is downloaded
  try {
    doc.save(`Transaction_Receipt_${productID}.pdf`);
